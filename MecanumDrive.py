...
class MecanumDrive(threading.Thread):
    def __init__(self,lx,ly,rx,ry):
        ...
        self.m1 = MC.Motor(12,2,3)
        #self.m1.start()
        self.m4 = MC.Motor(13,4,17)
        #self.m4.start()

        self.m3 = MC.Motor(18,14,15)
        #self.m3.start()
        self.m2 = MC.Motor(19,17,22)
        #self.m2.start()
        self.lx = lx
        self.ly = ly
        self.rx = rx
    ...
    def run(self):
        while True:
            ...
            if(left_x > .05):
                self.m1.setpower(self.m1.getpower()+(left_x*50))
                self.m2.setpower(self.m2.getpower()-(left_x*50))
                self.m3.setpower(self.m3.getpower()+(left_x*50))
                self.m4.setpower(self.m4.getpower()-(left_x*50))
            elif(left_x < -.05):
                self.m1.setpower(self.m1.getpower()+(left_x*50))
                self.m2.setpower(self.m2.getpower()-(left_x*50))
                self.m3.setpower(self.m3.getpower()+(left_x*50))
